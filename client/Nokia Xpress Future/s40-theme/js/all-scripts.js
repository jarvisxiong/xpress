...
function KScope(){
	...
	this.setStoriesListAndDetailView = function () {
	    ...
	    if(this.reqFilter === "WHATSHOT") {
	    	requestURL = serverURL + "/xpress/newsServlet";
	    ...
	    $.ajax({
		    ...
		    success: function(data){
		    	...
		    	if (data == undefined || data == null ||  data.length === 0){
		    		...
		    		news_provider=data.provider.value;
...
{
	var itemSitelogo=advertisement.value.replace(/[ ]/g, "");
	...
	outputstr += 'mwl.loadURL(\''+advertisement.link+'\');';
	outputstr += '">';
...
function getStoriesAndDetailPagesViewWithOptimization(data1, bShowNPS)
{
	...
	if(nextDataCount >= 10){
		...
		outputstr+='	<div id="right-navig-btn" class="active" onclick="mwl.addClass(\'#ui-navig-toolbar\', \'hide\'); mwl.switchClass(\'#toolbar-1\', \'show\', \'hide\'); mwl.switchClass(\'#toolbar-3\', \'hide\', \'show\');  Kismet.generateNextStoriesAndDetailPagesView();">'+locale.get('id_next')+'</div>';
		outputstr+='</div>';
	}
	if(prevDataCount > 0) {
		outputstr+='<div id="ui-navig-toolbar">';
		outputstr+='	<div id="left-navig-btn" class="active" onclick="mwl.addClass(\'#ui-navig-toolbar\', \'hide\'); mwl.switchClass(\'#toolbar-1\', \'show\', \'hide\'); mwl.switchClass(\'#toolbar-3\', \'hide\', \'show\');  Kismet.generatePrevStoriesAndDetailPagesView();">'+locale.get('id_previous')+'</div>';
...
function getStoriesAndDetailPagesView(data1, bShowNPS){
	...
	if(prevDataCount > 0) {
		outputstr+='<div id="ui-navig-toolbar">';
		outputstr+='	<div id="left-navig-btn" class="active" onclick="mwl.addClass(\'#ui-navig-toolbar\', \'hide\'); mwl.addClass(\'#ui-navig-toolbar\', \'hide\'); mwl.switchClass(\'#toolbar-1\', \'show\', \'hide\'); mwl.switchClass(\'#toolbar-3\', \'hide\', \'show\');  Kismet.generatePrevStoriesAndDetailPagesView();">'+locale.get('id_previous')+'</div>';
		outputstr+='	<div id="right-navig-btn" class="inactive">'+locale.get('id_next')+'</div>';
		outputstr+='</div>';
	}
	...
	this.getTileStoryViewHTMLStr = function()
	{
		...
		case "s2":
			if(this.item.image_url != null && this.item.image_url != undefined && this.item.image_url!=""
				...
				outputstr += '<div class="'+this.clsname+'titleImage" style="font-size:medium;">';
				if(this.item.title!=null){
					outputstr += getTheTextWithFinalCompleteWord(this.item.title, 4);//this.item.title.substr(0,10);
					if( this.item.title.length > 4){
						outputstr += " ...";
					}
				}
				outputstr += '</div>';
				...
				outputstr += '<div class="'+this.clsname+'link" style="color:'+this.txtclr+';font-size:medium;">';
				if(this.item.title!=null && this.item.title!=""){
					outputstr += '<b>';
					outputstr += getTheTextWithFinalCompleteWord(this.item.title, 18);//this.item.title.substr(0,38);
					if( this.item.title.length > 18){
						outputstr += ' ...';
					}
					outputstr += '</b>';
				}else {
					outputstr += getTheTextWithFinalCompleteWord(this.item.url,18);//this.item.url.substring(0,38);
					if( this.item.url.length > 18){
						outputstr += ' ...';
					}
				}
				outputstr += '</div>';
		...
		case "s3":
			...
			outputstr += '<div class="'+this.clsname+'title" style="font-size:medium;">';
			if(this.item.title!=null){
				outputstr += getTheTextWithFinalCompleteWord(this.item.title, 10);//this.item.title.substr(0,23);
				if( this.item.title.length > 10){
					outputstr += ' ...';
				}
			}
