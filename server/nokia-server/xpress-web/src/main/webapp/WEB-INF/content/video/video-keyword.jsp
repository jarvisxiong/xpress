...
<head>
	<title>NOKIA Video Keyword Management</title>
...
<div id="bd">
	...
	<div id="filter">
		<security:authorize ifAnyGranted="ROLE_EditVideo">
			<form id="inputForm" action="video-keyword!importFile.action" method="post" enctype="multipart/form-data">
				<input type="file" name="file"/>
				<input type="submit" value="Import" />
			</form>
		</security:authorize>
		<a href="/xpress/report/export.action?type=${type}">Export</a>
	</div>
	<form id="mainForm" action="video-keyword.action" method="post">
			...
			<table id="contentTable" width="100%">
				...
				<s:iterator value="page.result" status="rowStatus">
					<tr class=<s:if test="#rowStatus.odd">"oddRow"</s:if><s:else>"evenRow"</s:else>>
						...
						<td align="center">
						<security:authorize ifAnyGranted="ROLE_EditVideo">
								<a href="video-keyword!input.action?id=${id}">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<a href="video-keyword!delete.action?id=${id}"  onclick="return confirm('Do you want to delete it?')">Delete</a>
						</security:authorize>
						</td>
					</tr>
				</s:iterator>
		...
		<div>
			...
			<security:authorize ifAnyGranted="ROLE_EditVideo">
				<s:if test="type!=null">
					<a href="video-keyword!input.action">Add</a>
				</s:if>
			</security:authorize>
		</div>
	</form>
