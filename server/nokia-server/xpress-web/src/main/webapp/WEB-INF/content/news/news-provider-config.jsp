...
<head>
	<title>NOKIA News Provider Config Management</title>
	...
	<script>
		$(document).ready(function() {
			$("#menu-news-provider").addClass("active");
		});
	</script>
</head>
...
<div id="bd">
	<div id="yui-main">
	<div class="yui-b">
	<form id="mainForm" action="news-provider-config.action" method="post">
					...
					<tr class=<s:if test="#rowStatus.odd">"oddRow"</s:if><s:else>"evenRow"</s:else>>
						...
						<td align="center">
							<security:authorize ifAnyGranted="ROLE_EditNews">
								<a href="news-provider-config!input.action?id=${id}">Edit</a>&nbsp;&nbsp;
								<a href="news-provider-config!delete.action?id=${id}" onclick="return confirm('Do you want to delete it?')">Delete</a>
							</security:authorize>
						</td>
					</tr>
				</s:iterator>
		...
		<div>
			...
			<security:authorize ifAnyGranted="ROLE_EditNews">
				<s:if test="type!=null">
					<a href="news-provider-config!input.action">Add</a>
				</s:if>
			</security:authorize>
		</div>
	</form>
