...
<head>
	<title>NOKIA Reading Keyword Management</title>
	...
	<script>
		$(document).ready(function() {
			$("#menu-reading-keyword").addClass("active");
		});
		$(document).ready(function() {
			//聚焦第一个输入框
			$("#file").focus();
...
<div id="bd">
	...
	<div id="filter">
		<security:authorize ifAnyGranted="ROLE_EditReading">
			<form id="inputForm" action="reading-keyword!importFile.action" method="post" enctype="multipart/form-data">
				<input type="file" name="file"/>
				<input type="submit" value="Import" />
			</form>
		</security:authorize>
		<a href="reading-keyword!exportFile.action">Export</a>
	</div>
	<form id="mainForm" action="reading-keyword.action" method="post">
			...
			<table id="contentTable" width="100%">
				...
				<s:iterator value="page.result" status="rowStatus">
					<tr class=<s:if test="#rowStatus.odd">"oddRow"</s:if><s:else>"evenRow"</s:else>>
						...
						<td align="center">
						<security:authorize ifAnyGranted="ROLE_EditReading">
								<a href="reading-keyword!input.action?id=${id}">Edit</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<a href="reading-keyword!delete.action?id=${id}"  onclick="return confirm('Do you want to delete it?')">Delete</a>
						</security:authorize>
						</td>
					</tr>
				</s:iterator>
		...
		<div>
			...
			<security:authorize ifAnyGranted="ROLE_EditReading">
				<s:if test="type!=null">
					<a href="reading-keyword!input.action">Add</a>
				</s:if>
			</security:authorize>
		</div>
	</form>
