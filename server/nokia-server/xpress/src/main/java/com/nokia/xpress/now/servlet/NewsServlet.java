				...
				if (device != null && device.getEnable()) {
					...
					if (newsList != null && !newsList.isEmpty()) {
						...
						if (providerConfig != null) {
							...
							providerObject.put("value", providerConfig.getValue());
							jsonObject.put("provider", providerObject);
						}
						List<Advertisement> advertisementList = advertisementManager.findAllByType(TypeEnums.NEWS);
						if (advertisementList != null && !advertisementList.isEmpty())
							// jsonObject.put("advertisement", advertisementManager.parseAdvertisementListToJson(advertisementList));
							// 兼容一期的News接口
							jsonObject.put("advertisement", advertisementManager.parseAdvertisementToJson(advertisementManager.findAllByType(TypeEnums.NEWS).get(0)));
					}
				}
			}
